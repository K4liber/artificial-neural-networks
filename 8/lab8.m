...
%load data
for i = 1:inputsCount
    fileName = fullfile(pwd, strcat('Desktop/LSSN/8home/data/data', num2str(i), '.txt'));
    input = importdata(fileName, delimiterIn,headerlinesIn);
    inputs(:,i) = input(:);
    strcat('file ', num2str(i), '/', num2str(inputsCount))
end

targetsFileName = fullfile(pwd, 'Desktop/LSSN/8home/data/targets.txt');
